...
def delete_contact_by_email(event, context):
    try:
        body = json.loads(event.get('body', '{}'))
        ...
        conn = db_utils.get_db_connection()
        sql = "DELETE FROM email_agent.contacts WHERE email = :email"
        conn.run(sql, email=email)
        conn.run("COMMIT")
        conn.close()
        
        return {'statusCode': 200, 'body': json.dumps({'message': 'Contact deleted', 'email': email})}
    except Exception as e:
        print(f"DeleteContactByEmail Error: {e}")
        return {'statusCode': 500, 'body': json.dumps({'error': str(e)})}
