...
REDIRECT_URI = 'https://seksrdn8td.execute-api.us-east-2.amazonaws.com/auth/zoho/callback' 
...
def handle_callback(event):
    """Handle OAuth callback from Zoho - exchange code and redirect to frontend"""
    try:
        ...
        if not code:
            ...
            return {
                'statusCode': 302,
                'headers': {
                    'Location': 'http://localhost:3000?zoho_error=no_code'
                },
                'body': ''
            }
        
        # Redirect to frontend with code so it can call /auth/zoho/token
        return {
            'statusCode': 302,
            'headers': {
                'Location': f'http://localhost:3000/auth/callback?code={code}'
            },
            'body': ''
        }
        ...
        return {
            'statusCode': 302,
            'headers': {
                'Location': f'http://localhost:3000?zoho_error={str(e)}'
            },
            'body': ''
        }
