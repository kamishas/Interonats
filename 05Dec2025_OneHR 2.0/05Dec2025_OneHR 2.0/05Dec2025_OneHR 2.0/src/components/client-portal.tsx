...
export function ClientPortal() {
  ...
  const fetchClientData = async () => {
    try {
      ...
      const timesheetsRes = await fetch(`${CLIENT_API_URL}/client-portal/timesheets?clientId=${CLIENT_ID}`, {
      ...
      // Fetch invoices
      const invoicesRes = await fetch(`${CLIENT_API_URL}/client-portal/invoices?clientId=${CLIENT_ID}`, {
        headers: { 'Authorization': `Bearer ${getAccessToken() ?? ''}` },
      });
      if (invoicesRes.ok) {
  ...
  const handleApproveTimesheet = async (timesheetId: string, approved: boolean) => {
    try {
      const response = await fetch(`${TIMESHEET_API_URL}/timesheets/${timesheetId}/approve`, {
        method: 'POST',
        headers: {
