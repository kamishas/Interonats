...
export function HRDocumentWorkflow({ employee, onUpdate }: HRDocumentWorkflowProps) {
  ...
  useEffect(() => {
    const fetchWorkAuthDocuments = async () => {
      try {
        ...
        // Fetch full employee details which include documents
        const response = await fetch(`${API_URL}/employee/${employee.id}`, {
          headers: { 'Authorization': `Bearer ${token ?? ''}` }
        });
        if (response.ok) {
          const data = await response.json();
          // Extract documents from employee payload
          setWorkAuthDocuments(data.employee?.onboardingDocuments || []);
        }
      } catch (error) {
  ...
  const pendingDocuments = employee.onboardingDocuments?.filter(
    (doc: any) => doc.status === 'pending-review' || doc.status === 'pending_review' || doc.verificationStatus === 'pending'
  ) || [];
  ...
  const requestedDocuments = employee.onboardingDocuments?.filter(
    (doc: any) => (doc.status === 'not-uploaded' || doc.status === 'requested') && doc.type !== 'EAD' && 
    ...
    try {
      ...
      if (selectedDocuments.length > 0) {
        const response = await fetch(
          `${API_URL}/employee/${employee.id}/document-requests`,
          {
            method: 'POST',
            headers: {
      ...
      if (sendOfferLetterWithRequest) {
        const response = await fetch(
          `${API_URL}/onboarding/requests/${employee.id}/send-offer-letter`,
          {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token ?? ''}`,
      ...
      if (sendNDAWithRequest) {
        const response = await fetch(
          `${API_URL}/onboarding/requests/${employee.id}/send-nda`,
          {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token ?? ''}`,
  ...
  const eadDocument = employee.onboardingDocuments?.find(
    (doc: any) => doc.type === 'EAD' || doc.doc_type === 'ead' || doc.doc_type === 'work_authorization'
  );
  ...
  const ndaDocument = employee.onboardingDocuments?.find(
    (doc: any) => doc.type === 'nda' || doc.type === 'nda-signed' || doc.doc_type === 'nda'
  );
              ...
              <div className="space-y-2">
                <div className="flex items-start gap-3 p-2 rounded hover:bg-blue-100">
                  <Checkbox
                    ...
                    onCheckedChange={(checked: boolean) => setSendOfferLetterWithRequest(checked as boolean)}
                    disabled={!!offerLetterDocument?.status}
                  />
                  <div className="flex-1">
                ...
                <div className="flex items-start gap-3 p-2 rounded hover:bg-blue-100">
                  <Checkbox
                    ...
                    onCheckedChange={(checked: boolean) => setSendNDAWithRequest(checked as boolean)}
                    disabled={!!ndaDocument?.status}
                  />
                  <div className="flex-1">
