...
const API_URL = (import.meta as any).env.VITE_ONBOARDING_API_URL;
...
const generatePDFDocument = (doc: EmployeeDocument, employee: Employee) => {
  ...
  const metadata = [
    ...
    `Upload Date: ${safeFormat(doc.uploadDate, 'PPP')}`,
    `Uploaded By: ${doc.uploadedBy}`,
    doc.expiryDate ? `Expiry Date: ${safeFormat(doc.expiryDate, 'PPP')}` : '',
...
export function DocumentCollectionPanel({ employee, onDocumentUpdate }: DocumentCollectionPanelProps) {
  ...
  const normalizeDocType = (type: string, fileName?: string): string => {
    const t = (type || 'other').toLowerCase().trim().replace(/_/g, '-');
    
    // Explicit mapping
    if (t === 'ead' || t === 'work-authorization') return 'Work Authorization';
    if (t === 'government-id' || t === 'driver-license' || t === 'passport') return 'Government-issued ID';
    if (t === 'proof-of-address' || t === 'address-proof') return 'Address Proof';
    if (t === 'direct-deposit' || t === 'bank-account') return 'Direct Deposit Form'; // Added bank-account alias
    ...
    // Heuristics for 'other' or unknown types based on filename
    if (t === 'other' || t === 'unknown') {
        const f = (fileName || '').toLowerCase();
        if (f.includes('driver') || f.includes('license') || f.includes('passport') || f.includes('id')) return 'Government-issued ID';
        if (f.includes('utility') || f.includes('lease') || f.includes('address')) return 'Address Proof';
        if (f.includes('deposit') || f.includes('bank')) return 'Direct Deposit Form';
        if (f.includes('emergency')) return 'Emergency Contact';
        if (f.includes('i-9') || f.includes('i9')) return 'I-9';
        if (f.includes('w-4') || f.includes('w4')) return 'W-4';
        
        // CATCH-ALL: If it's an 'other' document and matches nothing else, 
        // assume it is the critical Work Authorization / EAD document.
        // This satisfies the requirement "Any doc is fine" for HR review.
        return 'Work Authorization';
    }
    
    if (!type) return 'Work Authorization'; // Default fallback for empty type
    return type; // Return original if defined but not mapped
  };
  ...
  const fetchData = async () => {
    try {
      ...
      if (docsRes.ok) {
        ...
        // Map backend snake_case to frontend camelCase and standardise statuses
        allDocuments = (data.documents || []).map((doc: any) => {
          const type = doc.doc_type || doc.documentType;
          const fName = doc.file_name || doc.fileName || doc.documentName || '';
          const normalizedType = normalizeDocType(type, fName);
          ...
          return {
            ...
            documentName: doc.document_name || fName || 'Untitled Document',
            fileName: fName,
            ...
            // verificationStatus is what the UI uses for the status badge (pending/verified/rejected)
            verificationStatus: (doc.status?.toLowerCase() === 'approved' || doc.status?.toLowerCase() === 'verified') ? 'verified' : 
                               (doc.status?.toLowerCase() === 'rejected') ? 'rejected' : 'pending',
            // status: 'active' is what the UI filters use to find "uploaded" documents
            status: 'active', 
            notes: doc.notes,
            uploadedBy: doc.uploaded_by || doc.uploadedBy || 'Employee',
            isWorkAuthDocument: normalizedType === 'Work Authorization' // Update this check too!
        };
        });
      } else {
      ...
      if (requestsRes.ok) {
        ...
        console.log('Document requests received from API:', rawRequests);
        ...
        const requests = rawRequests.map((r: any) => ({
            ...
            priority: r.priority,
            notes: r.notes,
            remindersSent: r.reminders_sent || r.remindersSent || 0,
  ...
  const handleViewDocument = async (doc: EmployeeDocument) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/employee/${employee.id}/documents/${doc.id}/download`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        console.log('[HR View] Loaded preview URL:', presigned_url);
        setDocumentPreviewUrl(presigned_url);
      } else {
        console.log('[HR View] No file available for preview');
        setDocumentPreviewUrl(null);
      }
    } catch (error) {
      console.error('[HR View] Error loading preview:', error);
      setDocumentPreviewUrl(null);
    } finally {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/employee/${employee.id}/documents`, {
        method: 'POST',
  ...
  const handleRequestDocument = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/employee/${employee.id}/document-requests`, {
        method: 'POST',
        headers: {
  ...
  const handleVerifyDocument = async (docId: string, status: 'verified' | 'rejected', reason?: string) => {
    try {
      ...
      const response = await fetch(`${API_URL}/employee/${employee.id}/documents/${docId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token ?? ''}`,
          'Content-Type': 'application/json',
                ...
                <div key={request.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      Due: {safeFormat(request.dueDate, 'MMM dd, yyyy')}
                ...
                <div key={docType.type} className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  {status.doc && (
                    <div className="flex gap-2">
                      ...
                      {(status.doc.verificationStatus === 'pending' || status.doc.verificationStatus === 'pending-review') && (
                        <>
                          <Button
                            size="sm"
                            variant="outline"
                  ...
                  {!status.doc && (
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        setUploadForm(prev => ({ ...prev, documentType: docType.type }));
              ...
              <TableBody>
                {documents.map(doc => (
                  <TableRow key={doc.id}>
                    ...
                    <TableCell>{safeFormat(doc.uploadDate, 'MMM dd, yyyy')}</TableCell>
                    <TableCell>
                      <Badge variant={
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        ...
                        {(doc.verificationStatus === 'pending' || doc.verificationStatus === 'pending-review') && (
                          <>
                            <Button
                              size="sm"
                              variant="outline"
                        ...
                        onClick={async () => {
                          try {
                            ...
                            const downloadResponse = await fetch(
                              `${API_URL}/employee/${employee.id}/documents/${selectedDocument.id}/download?filename=${encodeURIComponent(fileName)}`,
                              {
                                headers: {
                                  'Authorization': `Bearer ${token ?? ''}`
                                }
...
EMPLOYEE: ${employee.firstName} ${employee.lastName}
UPLOAD DATE: ${safeFormat(selectedDocument.uploadDate, 'PPP')}
${selectedDocument.expiryDate ? `EXPIRY DATE: ${safeFormat(selectedDocument.expiryDate, 'PPP')}` : ''}
...
<body>
  <div class="document">
    ...
    <div class="metadata">
      ...
      <div class="info"><span class="label">Upload Date:</span> ${safeFormat(selectedDocument.uploadDate, 'PPP')}</div>
      <div class="info"><span class="label">Uploaded By:</span> ${selectedDocument.uploadedBy}</div>
      ${selectedDocument.expiryDate ? `<div class="info"><span class="label">Expiry Date:</span> ${safeFormat(selectedDocument.expiryDate, 'PPP')}</div>` : ''}
      <div class="info">
      ...
      Generated on: ${new Date().toLocaleString()}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1">{safeFormat(selectedDocument.uploadDate, 'MMM dd, yyyy HH:mm')}</p>
                </div>
                <div>
                ...
                {selectedDocument.expiryDate && (
                  <div>
                    ...
                    <p className="mt-1">{safeFormat(selectedDocument.expiryDate, 'MMM dd, yyyy')}</p>
                  </div>
                )}
                <div>
